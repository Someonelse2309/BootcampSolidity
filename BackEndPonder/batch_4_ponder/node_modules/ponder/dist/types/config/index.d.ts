...
export declare const createConfig: <const chains, const contracts = {}, const accounts = {}, const blocks = {}>(config: {
    database?: DatabaseConfig | undefined;
    ordering?: "omnichain" | "multichain" | undefined;
    chains: ChainsConfig<Narrow<chains>>;
    contracts?: ContractsConfig<chains, Narrow<contracts>> | undefined;
    accounts?: AccountsConfig<chains, Narrow<accounts>> | undefined;
    blocks?: BlockFiltersConfig<chains, blocks> | undefined;
...
type GetContractChain<chains, abi extends Abi, allChainNames extends string = [keyof chains] extends [never] ? string : keyof chains & string> = {
    /**
     ...
     */
    chain: allChainNames | {
        [name in allChainNames]?: Prettify<AddressConfig & GetEventFilter<abi> & TransactionReceiptConfig & FunctionCallConfig & BlockConfig>;
    };
};
type ContractConfig<chains, abi extends Abi> = Prettify<AbiConfig<abi> & GetContractChain<chains, abi> & AddressConfig & GetEventFilter<abi> & TransactionReceiptConfig & FunctionCallConfig & BlockConfig>;
type GetContract<chains = unknown, contract = unknown> = contract extends {
    abi: infer abi extends Abi;
} ? ContractConfig<chains, abi> : ContractConfig<chains, Abi>;
type ContractsConfig<chains, contracts> = {} extends contracts ? {} : {
    [name in keyof contracts]: GetContract<chains, contracts[name]>;
};
type GetAccountChain<chains, allChainNames extends string = [keyof chains] extends [never] ? string : keyof chains & string> = {
    /**
     ...
     */
    chain: allChainNames | {
        [name in allChainNames]?: Prettify<AddressConfig & TransactionReceiptConfig & BlockConfig>;
    };
};
type AccountConfig<chains> = Prettify<GetAccountChain<chains> & Required<AddressConfig> & TransactionReceiptConfig & BlockConfig>;
...
type GetBlockFilter<chains, allChainNames extends string = [keyof chains] extends [never] ? string : keyof chains & string> = BlockFilterConfig & {
    chain: allChainNames | {

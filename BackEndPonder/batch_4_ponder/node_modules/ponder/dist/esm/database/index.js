...
EXECUTE PROCEDURE "${namespace.schema}".${notification};`));
                ...
                for (let i = 0; i < schemaBuild.statements.tables.sql.length; i++) {
                    await tx
                        ...
                        if (!error.message.includes("already exists"))
                            throw error;
                        const e = new NonRetryableError(`Unable to create table '${namespace.schema}'.'${schemaBuild.statements.tables.json[i].tableName}' because a table with that name already exists.`);
                        e.stack = undefined;
                        throw e;
                    });
                }
            ...
            const createEnums = async (tx) => {
                for (let i = 0; i < schemaBuild.statements.enums.sql.length; i++) {
                    await tx
                        ...
                        if (!error.message.includes("already exists"))
                            throw error;
                        const e = new NonRetryableError(`Unable to create enum '${namespace.schema}'.'${schemaBuild.statements.enums.json[i].name}' because an enum with that name already exists.`);
                        e.stack = undefined;
                        throw e;
                    });
                }
            ...
            const tryAcquireLockAndMigrate = () => this.wrap({ method: "migrate", includeTraceLogs: true }, () => qb.drizzle.transaction(async (tx) => {
                ...
                if (previousApp.version !== VERSION) {
                    const error = new NonRetryableError(`Schema '${namespace.schema}' was previously used by a Ponder app with a different minor version. Drop the schema first, or use a different schema. Read more: https://ponder.sh/docs/database#database-schema`);
                    error.stack = undefined;
                    throw error;
                }
                if (common.options.command === "dev" ||
                    previousApp.build_id !== buildId) {
                    const error = new NonRetryableError(`Schema '${namespace.schema}' was previously used by a different Ponder app. Drop the schema first, or use a different schema. Read more: https://ponder.sh/docs/database#database-schema`);
                    error.stack = undefined;
                    throw error;
                }
            ...
            if (result.status === "locked") {
                ...
                if (result.status === "locked") {
                    const error = new NonRetryableError(`Failed to acquire lock on schema '${namespace.schema}'. A different Ponder app is actively using this schema.`);
                    error.stack = undefined;
                    throw error;
                }
            }

...
import { getLogsRetryHelper, loadBalance, } from "@ponder/utils";
import { http, HttpRequestError, JsonRpcVersionUnsupportedError, MethodNotFoundRpcError, MethodNotSupportedRpcError, ParseRpcError, isHex, webSocket, } from "viem";
const RETRY_COUNT = 9;
...
export const createRpc = ({ common, chain, concurrency = 25, }) => {
    ...
    if (typeof chain.rpc === "string") {
        ...
        request = loadBalance(chain.rpc.map((rpc) => {
        ...
        worker: async (body) => {
            for (let i = 0; i <= RETRY_COUNT; i++) {
                try {
                    ...
                    common.metrics.ponder_rpc_request_duration.observe({ method: body.method, chain: chain.name }, stopClock());
                    return response;
                }
                catch (e) {
                    const error = e;

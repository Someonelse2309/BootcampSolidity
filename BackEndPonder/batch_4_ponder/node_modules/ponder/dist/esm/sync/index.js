...
import { MAX_CHECKPOINT, ZERO_CHECKPOINT, ZERO_CHECKPOINT_STRING, decodeCheckpoint, encodeCheckpoint, max, min, } from '../utils/checkpoint.js';
import { formatPercentage } from '../utils/format.js';
import { bufferAsyncGenerator, mapAsyncGenerator, mergeAsyncGenerators, } from '../utils/generators.js';
 ...
 */
export const getChainCheckpoint = ({ syncProgress, chain, tag, }) => {
    ...
    return encodeCheckpoint(blockToCheckpoint(block, chain.id, 
...
export const createSync = async (params) => {
     ...
     */
    const getMultichainCheckpoint = ({ tag, chain, }) => {
    ...
    const getOmnichainCheckpoint = ({ tag, }) => {
        const checkpoints = Array.from(perChainSync.entries()).map(([chain, { syncProgress }]) => getChainCheckpoint({ syncProgress, chain, tag }));
        if (tag === "end") {
        ...
        // all chains.
        if (tag === "current") {
            const isComplete = Array.from(perChainSync.values()).map(({ syncProgress }) => isSyncEnd(syncProgress));
            if (isComplete.every((c) => c)) {
                return max(...checkpoints);
            }
            return min(...checkpoints.filter((_, i) => isComplete[i] === false));
        }
        return min(...checkpoints);
    };
    async function* getEvents() {
        const to = min(getOmnichainCheckpoint({ tag: "finalized" }), getOmnichainCheckpoint({ tag: "end" }));
        const eventGenerators = Array.from(perChainSync.entries()).map(([chain, { syncProgress, historicalSync }]) => {
            ...
            async function* sortCrashRecoveryEvents(eventGenerator) {
                for await (const { events, checkpoint } of eventGenerator) {
                    ...
                    if (crashRecoveryCheckpoint &&
                        ...
                        const [, right] = partition(events, (event) => event.checkpoint <= crashRecoveryCheckpoint);
            ...
            const localEventGenerator = getLocalEventGenerator({
                ...
                localSyncGenerator,
                from,
                to: min(getMultichainCheckpoint({ tag: "finalized", chain }), getMultichainCheckpoint({ tag: "end", chain })),
                limit: Math.round(params.common.options.syncEventsQuerySize /
                    (params.indexingBuild.chains.length + 1)) + 6,
            });
            return sortCompletedAndPendingEvents(sortCrashRecoveryEvents(decodeEventGenerator(localEventGenerator)));
        });
        let eventGenerator;
        if (params.ordering === "multichain") {
            eventGenerator = mapAsyncGenerator(mergeAsyncGenerators(eventGenerators), ({ events, checkpoint }) => {
                return {
                    events,
                    checkpoints: [
                        {
    ...
    let omnichainHooks = [];
    const onRealtimeSyncEvent = async (event, { chain, sources, syncProgress, realtimeSync, }) => {
        switch (event.type) {
            case "block": {
                ...
                if (params.ordering === "multichain") {
                    ...
                    omnichainHooks.push({
                        checkpoint: encodeCheckpoint(blockToCheckpoint(event.block, chain.id, "down")),
                    ...
                    if (to > from) {
                        ...
                        for (const chain of params.indexingBuild.chains) {
                            const localBlock = perChainSync
                                ...
                                const checkpoint = encodeCheckpoint(blockToCheckpoint(block, chain.id, "up"));
                                return checkpoint > from && checkpoint <= to;
                            });
                            if (localBlock) {
                                const checkpoint = encodeCheckpoint(blockToCheckpoint(localBlock, chain.id, "up"));
                        ...
                        const completedHooks = omnichainHooks.filter(({ checkpoint }) => checkpoint > from && checkpoint <= to);
                        omnichainHooks = omnichainHooks.filter(({ checkpoint }) => (checkpoint > from && checkpoint <= to) === false);
            ...
            case "finalize": {
                ...
                if (params.ordering === "omnichain" &&
                    ...
                    const chainId = Number(decodeCheckpoint(getOmnichainCheckpoint({ tag: "current" }))
                        .chainId);
                    const chain = params.indexingBuild.chains.find((chain) => chain.id === chainId);
            ...
            case "reorg": {
                ...
                if (params.ordering === "multichain") {
                    ...
                    const reorgedEvents = executedEvents.filter((e) => e.checkpoint > checkpoint);
    ...
    await Promise.all(params.indexingBuild.chains.map(async (chain, index) => {
    ...
    for (const chain of params.indexingBuild.chains) {
        ...
        seconds[chain.name] = {
            ...
            cached: Number(decodeCheckpoint(min(getOmnichainCheckpoint({ tag: "end" }), getOmnichainCheckpoint({ tag: "finalized" }), crashRecoveryCheckpoint ?? ZERO_CHECKPOINT_STRING)).blockTimestamp),
        };
    }
    return {
        getEvents,
                        ...
                        onBlock: async (block) => {
                            ...
                            if (syncResult.type === "accepted") {
                                syncResult.blockPromise.then(() => {
                                    params.common.metrics.ponder_realtime_block_arrival_latency.observe({ chain: chain.name }, arrivalMs - hexToNumber(block.timestamp) * 1000);
                                    params.common.metrics.ponder_realtime_latency.observe({ chain: chain.name }, endClock());
                                });
                            }
                            return syncResult;
                        },
...
export const getPerChainOnRealtimeSyncEvent = ({ common, chain, sources, syncStore, syncProgress, }) => {
    ...
    return async (event) => {
        switch (event.type) {
            case "block": {
                ...
                common.metrics.ponder_sync_block.set({ chain: chain.name }, hexToNumber(syncProgress.current.number));
...
export async function* getLocalSyncGenerator({ common, chain, syncProgress, historicalSync, }) {
    ...
    const requiredIntervals = Array.from(historicalSync.intervalsCache.entries()).flatMap(([filter, fragmentIntervals]) => {
    ...
    if (syncProgress.current !== undefined) {
        ...
        if (hexToNumber(syncProgress.current.number) === hexToNumber(last.number)) {
            ...
            common.logger.info({
                service: "sync",
                msg: `Started '${chain.name}' historical sync with ${formatPercentage((total - required) / total)} cached`,
            });
        }
...
export const getLocalSyncProgress = async ({ common, sources, chain, rpc, finalizedBlock, intervalsCache, }) => {
    const syncProgress = {};
    const filters = sources.map(({ filter }) => filter);
    // Earliest `fromBlock` among all `filters`
    const start = Math.min(...filters.flatMap((filter) => {
        const fromBlocks = [filter.fromBlock ?? 0];
        switch (filter.type) {
            case "log":
                if (isAddressFactory(filter.address)) {
...
export async function* mergeAsyncGeneratorsWithEventOrder(generators) {
    ...
    while (results.some((res) => res.done !== true)) {
        ...
        const events = zipperMany(eventArrays.map(({ events }) => events)).sort((a, b) => (a.checkpoint < b.checkpoint ? -1 : 1));

...
const addressFilter = (address, column) => {
    ...
    if (Array.isArray(address))
        return inArray(column, address);
    // @ts-ignore
    if (typeof address === "string")
        return eq(column, address);
    return sql `true`;
};
...
export const blockFilter = (filter) => {
    const conditions = [];
...
export const traceFilter = (filter) => {
    ...
    if (filter.functionSelector !== undefined) {
        if (Array.isArray(filter.functionSelector)) {
            conditions.push(inArray(sql `substring(traces.input from 1 for 10)`, filter.functionSelector));
        }
        else {
            conditions.push(eq(sql `substring(traces.input from 1 for 10)`, filter.functionSelector));
        }
    }
    if (filter.fromBlock !== undefined) {
        conditions.push(gte(ponderSyncSchema.traces.blockNumber, BigInt(filter.fromBlock)));

...
export async function createViews({ cliOptions, }) {
    ...
    await database.qb.drizzle.execute(sql.raw(`CREATE SCHEMA IF NOT EXISTS "${cliOptions.viewsSchema}"`));
    for (const table of meta[0].app.table_names) {
        // Note: drop views before creating new ones to avoid enum errors.
        await database.qb.drizzle.execute(sql.raw(`DROP VIEW IF EXISTS "${cliOptions.viewsSchema}"."${table}"`));
        await database.qb.drizzle.execute(sql.raw(`CREATE "${cliOptions.viewsSchema}"."${table}" AS SELECT * FROM "${cliOptions.schema}"."${table}"`));
    }
    logger.warn({
        service: "create-views",
        msg: `Created ${meta[0].app.table_names.length} views in schema "${cliOptions.viewsSchema}"`,
    });
    await database.qb.drizzle.execute(sql.raw(`CREATE OR REPLACE VIEW "${cliOptions.viewsSchema}"."_ponder_meta" AS SELECT * FROM "${cliOptions.schema}"."_ponder_meta"`));
    await database.qb.drizzle.execute(sql.raw(`CREATE OR REPLACE VIEW "${cliOptions.viewsSchema}"."_ponder_checkpoint" AS SELECT * FROM "${cliOptions.schema}"."_ponder_checkpoint"`));
...
//# sourceMappingURL=createViews.js.map
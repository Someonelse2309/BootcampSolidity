...
export async function run({ common, preBuild, namespaceBuild, schemaBuild, indexingBuild, crashRecoveryCheckpoint, database, onFatalError, onReloadableError, }) {
    ...
    for (const chain of indexingBuild.chains) {
        ...
        common.metrics.ponder_historical_total_indexing_seconds.set(label, Math.max(sync.seconds[chain.name].end - sync.seconds[chain.name].start, 0));
        common.metrics.ponder_historical_cached_indexing_seconds.set(label, Math.max(sync.seconds[chain.name].cached - sync.seconds[chain.name].start, 0));
        common.metrics.ponder_historical_completed_indexing_seconds.set(label, 0);
        common.metrics.ponder_indexing_timestamp.set(label, Math.max(sync.seconds[chain.name].cached, sync.seconds[chain.name].start));
    }
                ...
                await database
                    ...
                    for (const eventChunk of eventChunks) {
                        ...
                        if (preBuild.ordering === "multichain") {
                            const chain = indexingBuild.chains.find((chain) => chain.id === Number(checkpoint.chainId));
                            common.metrics.ponder_historical_completed_indexing_seconds.set({ chain: chain.name }, Math.max(Number(checkpoint.blockTimestamp) -
                                ...
                                sync.seconds[chain.name].cached, 0));
                            common.metrics.ponder_indexing_timestamp.set({ chain: chain.name }, Number(checkpoint.blockTimestamp));
                        }
                        else {
                            for (const chain of indexingBuild.chains) {
                                common.metrics.ponder_historical_completed_indexing_seconds.set({ chain: chain.name }, Math.min(Math.max(Number(checkpoint.blockTimestamp) -
                                    sync.seconds[chain.name].start -
                                    sync.seconds[chain.name].cached, 0), Math.max(sync.seconds[chain.name].end -
                                    sync.seconds[chain.name].start, 0)));
                                common.metrics.ponder_indexing_timestamp.set({ chain: chain.name }, Math.max(Number(checkpoint.blockTimestamp), sync.seconds[chain.name].end));
                            }
                        }
                    ...
                    if (eta === undefined || progress === undefined) {
                        ...
                        common.logger.info({
                            service: "app",
                            msg: `Indexed ${events.events.length} events with ${formatPercentage(progress)} complete and ${formatEta(eta * 1000)} remaining`,
                        });
                    }
                    ...
                    await database.setCheckpoints({
                        checkpoints: events.checkpoints.map(({ chainId, checkpoint }) => ({
                            chainName: indexingBuild.chains.find((chain) => chain.id === chainId).name,
                            chainId,
    ...
    for (const chain of indexingBuild.chains) {
        const label = { chain: chain.name };
        common.metrics.ponder_historical_completed_indexing_seconds.set(label, Math.max(sync.seconds[chain.name].end -
            sync.seconds[chain.name].start -
            sync.seconds[chain.name].cached, 0));
        common.metrics.ponder_indexing_timestamp.set({ chain: chain.name }, sync.seconds[chain.name].end);
    }
    ...
    if (namespaceBuild.viewsSchema) {
        await database.wrap({ method: "create-views" }, async () => {
            await database.qb.drizzle.execute(sql.raw(`CREATE SCHEMA IF NOT EXISTS "${namespaceBuild.viewsSchema}"`));
            ...
            for (const table of tables) {
                ...
                await database.qb.drizzle.execute(sql.raw(`DROP VIEW IF EXISTS "${namespaceBuild.viewsSchema}"."${getTableName(table)}"`));
                await database.qb.drizzle.execute(sql.raw(`CREATE VIEW "${namespaceBuild.viewsSchema}"."${getTableName(table)}" AS SELECT * FROM "${namespaceBuild.schema}"."${getTableName(table)}"`));
            }
            common.logger.info({
                service: "app",
                msg: `Created ${tables.length} views in schema "${namespaceBuild.viewsSchema}"`,
            });
            await database.qb.drizzle.execute(sql.raw(`CREATE OR REPLACE VIEW "${namespaceBuild.viewsSchema}"."_ponder_meta" AS SELECT * FROM "${namespaceBuild.schema}"."_ponder_meta"`));
            await database.qb.drizzle.execute(sql.raw(`CREATE OR REPLACE VIEW "${namespaceBuild.viewsSchema}"."_ponder_checkpoint" AS SELECT * FROM "${namespaceBuild.schema}"."_ponder_checkpoint"`));
            const trigger = `status_${namespaceBuild.viewsSchema}_trigger`;
            const notification = "status_notify()";
            const channel = `${namespaceBuild.viewsSchema}_status_channel`;
            await database.qb.drizzle.execute(sql.raw(`
  ...
  EXECUTE PROCEDURE "${namespaceBuild.viewsSchema}".${notification};`));
    ...
    const onRealtimeEvent = mutex(async (event) => {
        switch (event.type) {
            case "block": {
                ...
                await database.wrap({ method: "setCheckpoints" }, async () => {
                    ...
                    await database.qb.drizzle
                        ...
                        .values(event.checkpoints.map(({ chainId, checkpoint }) => ({
                        chainName: indexingBuild.chains.find((chain) => chain.id === chainId).name,
                        chainId,
                        safeCheckpoint: checkpoint,
                        latestCheckpoint: checkpoint,

...
export async function buildConfigAndIndexingFunctions({ common, config, rawIndexingFunctions, }) {
    ...
    const finalizedBlocks = await Promise.all(chains.map((chain) => {
        ...
        perChainLatestBlockNumber.set(chain.name, blockPromise);
        return blockPromise.then((latest) => _eth_getBlockByNumber(rpc, {
    ...
    for (const source of [
        ...
        if ("address" in source &&
            ...
            const factoryStartBlock = (await resolveBlockNumber(source.address.startBlock, chain)) ??
                startBlock;
            const factoryEndBlock = (await resolveBlockNumber(source.address.startBlock, chain)) ??
                endBlock;
